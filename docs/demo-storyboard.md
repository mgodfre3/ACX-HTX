...
| 2 | Left shell: `cat /var/lib/edge-fetch/videos/sample-video-01/envelope.json \| jq '.kek_ref, .wrap_algo'`. Shows `"transit/keys/htx-kek"` and `"vault-transit-rsa-3072"` (RSA-3072 asymmetric key held on-prem; the DEK is wrapped with it via Vault Transit). Older envelopes seeded before 2026-09-14 may show `"vault-transit-aes256gcm96"` -- that was a stale hardcoded label; the actual key type has always been RSA-3072. | *"The key that can unlock it lives in a Vault instance right next to it. This key has never been in Azure. It never will be."* |
| 3 | Left shell (presenter runs this via ssh): `VAULT_ADDR=http://127.0.0.1:8200 VAULT_TOKEN=$(cat /etc/vault/htx-toggle-token) vault read -field=type transit/keys/htx-kek`. Prints one line: `rsa-3072`. Confirms the key exists locally. | *"Same customer, same data center, same physical enclosure. RSA-3072, on-prem, never leaves."* |
